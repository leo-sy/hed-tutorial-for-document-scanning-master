...
flags.DEFINE_string('image', './test_image/003.jpeg', 
...
if __name__ == "__main__":
    ...
    m,n=img.shape[:2]
    name='trans_'+FLAGS.image.split('/')[-1]
    ...
    with tf.Session() as sess:
        ...
        dsn_fuse_image_path = os.path.join(FLAGS.output_dir, name)
        
        
        cv2.imwrite(dsn_fuse_image_path, dsn_fuse_image)

        img=cv2.imread(dsn_fuse_image_path)
        img=cv2.resize(img,(n,m))
        cv2.imwrite(dsn_fuse_image_path, img)
